...
import type { InviteLookup } from "@/lib/data-bundles/invites";
import { inviteProblemMessage, inviteWaiverLabel } from "@/lib/data-bundles/invite-rules";

/**
 * The registration page itself, minus how it was reached.
 *
 * Two routes render it: `/become-an-agent`, which reads the code out of the
 * query string, and `/<short path>`, which resolves the same link from a bare
 * path an advert can carry. Both hand in an already-resolved link, so the page
 * cannot end up pricing one thing on one route and another on the other — and
 * the code goes into the form the same way either way, which is what makes the
 * submit path identical too.
 */
...
export async function AgentRegistration({
  ...
  /** The recruiter's agent code from `?ref=`, normalised, or "". */
  referralCode: string;
  /** The Nickimart link this applicant arrived on, already resolved. */
  ...
  // buys nothing.
  const invitedBy = referralCode;

  // A registration link Nickimart issued itself. Resolved by the caller so the
  // fee on the form is the fee they will be charged — and re-resolved on
  // submit, so a link edited in the address bar buys nothing.
