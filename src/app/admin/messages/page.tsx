...
/**
 * Every automatic message the mall sends, and the words in it.
 *
 ...
 */
export default async function AdminRetailMessagesPage() {
  const [templates] = await Promise.all([listTemplates("retail")]);
  const groups = templateGroups("retail");
          ...
          <ul className="space-y-2">
            {templates
              ...
              .map((t) => (
                <li key={t.key}>
                  <ActionLink
                    href={`/admin/messages/${encodeURIComponent(t.key)}`}
