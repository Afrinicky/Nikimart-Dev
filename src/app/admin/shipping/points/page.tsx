...
export const metadata: Metadata = { title: "Consolidation points — Shipping — Admin" };
export const dynamic = "force-dynamic";

/**
 * Every place a load is gathered, local and international in one list.
 *
 * They were two lists before — pickup points and "arrival points" — and the
 * same warehouse had to be entered twice to be both. The column that matters
 * most here is "Sits at", because a point at a pickup station is what makes
 * collecting there free.
 */
export default async function ConsolidationPointsPage() {
  const points = await prisma.arrivalPoint.findMany({
    ...
    include: {
      ...
      _count: { select: { products: true, rulesFrom: true, forwarders: true } },
    },
  });
  ...
  return (
    <Container className="py-8">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-xl font-bold text-niki-ink">Consolidation points</h1>
          <p className="mt-1 max-w-2xl text-sm text-niki-ink/60">
            Where goods are gathered and checked before a courier takes them to a buyer&apos;s
            station. A point that sits at a pickup station makes collection there free.
          </p>
        </div>
        <Link
      ...
      {points.length === 0 ? (
        <div className="mt-6 rounded-2xl bg-white p-8 text-center ring-1 ring-niki-edge">
          ...
          <p className="mt-3 font-semibold text-niki-ink">No consolidation points yet</p>
          <p className="mx-auto mt-1 max-w-md text-sm text-niki-ink/60">
            Until one exists, every listing prices from the platform defaults and no collection is
            ever free — a buyer would be charged to collect from the shelf their goods sit on.
          </p>
          <Link
            href="/admin/shipping/points/new"
        ...
        <div className="mt-6 overflow-x-auto rounded-2xl bg-white ring-1 ring-niki-edge">
          <table className="w-full min-w-[820px] text-left text-sm">
            ...
            <tbody className="divide-y divide-niki-edge">
              {points.map((p) => {
                const international = isPointKind(p.kind) ? p.kind === "international" : true;
                return (
                  <tr key={p.id}>
                    ...
                    <td className="px-5 py-3">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-semibold ${
                          international ? "bg-niki-orange/10 text-niki-orange" : "bg-niki-black/5 text-niki-ink/70"
                        }`}
                      >
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <DeleteButton
                          ...
                          title={
                            p._count.products > 0 || p._count.forwarders > 0
                              ? "In use — will be retired, not deleted"
                              : undefined
                          }
                        />
                      </div>
