    ...
    <div>
        ...
        <div className="mb-3 grid grid-cols-3 gap-3 sm:grid-cols-4">
          {urls.map((url, i) => (
            <div key={`${url.slice(0, 24)}-${i}`} className="group relative overflow-hidden rounded-xl ring-1 ring-niki-edge-strong">
              ...
              {i === 0 ? (
                <span className="absolute left-1 top-1 flex items-center gap-1 rounded-lg bg-niki-orange px-1.5 py-0.5 text-[9px] font-bold text-white">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button
          ...
          disabled={busy}
          className="flex items-center gap-2 rounded-lg bg-niki-black px-4 py-2 text-xs font-semibold text-white transition-colors hover:bg-niki-black-soft disabled:opacity-60"
        >
        ...
        <div className="flex flex-1 items-center gap-2">
          ...
          />
          <button type="button" onClick={addUrl} className="niki-chip flex items-center gap-1 rounded-lg px-3 py-2 text-xs font-semibold text-niki-ink/75 hover:text-niki-ink">
